...
mod tests {
    ...
    fn iri_to_uri() {
        for (iri, expected_uri) in CASES {
            ...
            let iri = IriReferenceString::try_from(*iri).expect("test cases must be valid");
            let uri1 = iri.encode_to_uri().to_dedicated_string();
            assert_eq!(uri1, expected_uri);
            let uri2 = IriReferenceString::from(uri1)
                .encode_to_uri()
                .to_dedicated_string();
            assert_eq!(uri2, expected_uri, "`encode_to_uri` must be idempotent");
        }
    }
