...
//!
//! See [RFC 3986 section 5](https://tools.ietf.org/html/rfc3986#section-5).
...
use crate::{
    parser::IriReferenceComponents,
    types::{AbsoluteIriStr, IriReferenceStr, IriString},
};
...
fn resolve_impl(reference: &IriReferenceStr, base: &AbsoluteIriStr, is_strict: bool) -> IriString {
    let r = IriReferenceComponents::from(reference);
    let b = IriReferenceComponents::from(base.as_ref());
    let b_scheme = b
        .scheme
        .expect("Should never fail: `AbsoluteIriStr` should have `scheme` part");
...
fn recompose_components(
    ...
    IriString::try_from(s).unwrap_or_else(|e| {
        panic!(
